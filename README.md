...
> **About the demo:** Stride is designed as a **privacy-first personal app** — there is no authentication system (out of scope for this hackathon). The live demo runs on a single shared instance, which means **all visitors share the same profile data**. Feel free to explore, but be aware that others may be using it at the same time. The demo by default uses **Mistral** (`ministral-3b-2512`) as LLM and **Mistral Voxtral** (`voxtral-mini-2602`) for speech-to-text, but you can also switch to **Groq** or **Google Gemini** via the Settings page. API keys on the live demo are temporary and will be deactivated after the hackathon. To get the full experience, please deploy the app locally and use your own API keys (see `.env.example`). The Google Maps API key expires on **February 19, 2026** — after that date, the Jobs tab will not return location results.
