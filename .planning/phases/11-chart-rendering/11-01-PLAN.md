...
must_haves:
  ...
  artifacts:
    - path: "packages/frontend/src/components/chat/OnboardingChat.tsx"
      provides: "QUICK_LINKS array with 4 entries and proper chartType-to-action mapping"
      contains: "savings"
    - path: "packages/frontend/src/lib/chat/intent/detector.ts"
      provides: "Intent detection patterns for chart requests"
      contains: "show_budget_chart"
  key_links:
    - from: "OnboardingChat.tsx handleUIAction show_chart"
      to: "chat.ts handleConversationMode"
      via: "fetch /api/chat with chartType"
      pattern: "show_chart.*chartType"
    - from: "chat.ts intent detection"
      to: "show_*_chart action"
      via: "detectIntent()"
      pattern: "show_budget_chart|show_progress_chart|show_energy_chart|show_projection_chart"
---
...
@packages/frontend/src/lib/chatChartBuilder.ts (chart building functions)
@packages/frontend/src/lib/chat/intent/detector.ts (intent patterns)
</context>
