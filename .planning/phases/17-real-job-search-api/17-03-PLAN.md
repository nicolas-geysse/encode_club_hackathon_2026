...
must_haves:
  ...
  artifacts:
    ...
    - path: "packages/frontend/src/components/prospection/ProspectionSwipeDeck.tsx"
      ...
      contains: "StarRating"
    - path: "packages/frontend/src/lib/prospectionTypes.ts"
      provides: "Extended props with skills and rate"
      contains: "userSkills"
  key_links:
    - from: "plan.tsx"
      to: "ProspectionTab.tsx"
      via: "prop passing"
      pattern: "userSkills=.*activeProfile.*skills"
...
Purpose: JOBS-02 (display name/address/rating), JOBS-04 (skill matching), JOBS-05 (star rating based on scoring)
...
<task type="auto">
  <name>Task 1: Add scoring to ProspectionTab flow and wire props from plan.tsx</name>
  <files>
    packages/frontend/src/lib/prospectionTypes.ts
    packages/frontend/src/components/tabs/ProspectionTab.tsx
    packages/frontend/src/routes/plan.tsx
  ...
  <action>
Integrate job scoring into the prospection search flow and wire skills data from parent.

1. Update ProspectionTabProps in prospectionTypes.ts (around line 168-177):
   ```typescript
...
2. Add import at top of ProspectionTab.tsx:
   ```typescript
...
6. **CRITICAL: Wire props from plan.tsx to ProspectionTab** (around line 677-689):
   Update the ProspectionTab usage in plan.tsx to pass userSkills and minHourlyRate:
   ```tsx
   <ProspectionTab
   ...
   />
   ```
   The FullProfile type (from profileService.ts) already has `skills?: string[]` and `minHourlyRate?: number` fields.
  </action>
  <verify>
`pnpm typecheck` passes with the new ScoredJob type usage and prop wiring.
  </verify>
  <done>
ProspectionTab receives skills/rate from plan.tsx, scores jobs using the algorithm, and passes scored cards to SwipeDeck.
  </done>
</task>
