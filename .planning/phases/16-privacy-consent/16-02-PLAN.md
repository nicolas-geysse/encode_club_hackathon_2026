...
must_haves:
  ...
  key_links:
    ...
    - from: "OnboardingChat.tsx"
      ...
      via: "fuzzyCoordinates for forwardGeocode results"
...
7. **Apply fuzzyCoordinates to forwardGeocode results (manual city entry):**
When user enters city manually and handleCitySubmit calls forwardGeocode, wrap the result:
```typescript
// In handleCitySubmit or wherever forwardGeocode is called:
const geocodeResult = await forwardGeocode(cityName);
if (geocodeResult && geocodeResult.latitude && geocodeResult.longitude) {
  const fuzzy = fuzzyCoordinates(geocodeResult.latitude, geocodeResult.longitude);
  updateProfileData({
    city: cityName,
    latitude: fuzzy.latitude,
    longitude: fuzzy.longitude,
...
```
Find the existing forwardGeocode call site and add the fuzzyCoordinates wrapper there.
...
- Fuzzy coordinates are used when storing location data from GPS
- Fuzzy coordinates are used when storing location data from forwardGeocode (manual entry)
  </done>
</task>
...
1. **Add sanitization function directly in opik.ts:**
Since opik.ts is in mcp-server and locationPrivacy.ts is in frontend, we duplicate for hackathon speed.
...
5. **Add JSDoc comment at top of sanitization section:**
```typescript
/**
 * PRIVACY COMPLIANCE:
...
```
  ...
  <done>
- opik.ts has sanitizeLocationPII function with dual-maintenance warning
...
- Manual city entry (forwardGeocode) also applies fuzzyCoordinates
- profiles.ts enforces fuzzy coordinates on all writes
- opik.ts sanitizes all inputs/outputs for location PII with maintenance warning
